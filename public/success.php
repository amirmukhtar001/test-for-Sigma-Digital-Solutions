...
if (isset($_GET['session_id'])) {
    ...
    // PayTabs payment success
    $paymentData = $_SESSION['payment_data'];

    $sql = "INSERT INTO payments (user_id, transaction_id, amount, payment_method, status) VALUES (?, ?, ?, ?, ?)";
    if ($stmt = mysqli_prepare($link, $sql)) {
        $status = 'completed';
        mysqli_stmt_bind_param($stmt, "isdss", $paymentData['user_id'], $paymentData['transaction_id'], $paymentData['amount'], $paymentData['payment_method'], $status);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
